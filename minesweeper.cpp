...
//	None	
void initField(int num_of_mine) {
	//Complete me// Initialize all cells to UNKNOWN
    c00 = c01 = c02 = c03 = c04 = c10 = c11 = c12 = c13 = c14 = UNKNOWN;
    c20 = c21 = c22 = c23 = c24 = c30 = c31 = c32 = c33 = c34 = UNKNOWN;
    c40 = c41 = c42 = c43 = c44 = c50 = c51 = c52 = c53 = c54 = UNKNOWN;
	...
	if (!isMine(i, j)) {
        ...
        for (int row = i - 1; row <= i + 1; row++) {
            for (int col = j - 1; col <= j + 1; col++) {
                if (row >= 0 && row < height && col >= 0 && col < width && isMine(row, col)) {
                    count++;
                }
            }
        }
        // Update the cell value to show the number of surrounding mines
        setCell(i, j, count);

        // If there are no surrounding mines, reveal surrounding cells recursively
        if (count == 0) {
            for (int row = i - 1; row <= i + 1; row++) {
                for (int col = j - 1; col <= j + 1; col++) {
                    if (row >= 0 && row < height && col >= 0 && col < width && getCell(row, col) == UNKNOWN) {
                        reveal(row, col);
                    }
                }
            }
...
if (action == 'f') {
        ...
        if (getCell(row, col) == UNFLAGGED_MINE) {
            setCell(row, col, FLAGGED_MINE);
            num_correct_flagged_mine++;
        } else if (getCell(row, col) == FLAGGED_MINE) {
            setCell(row, col, UNKNOWN); // Change FLAGGED_MINE back to UNKNOWN
            num_correct_flagged_mine--;
        } else if (getCell(row, col) == INCORRECT_FLAGGED_MINE) {
            setCell(row, col, UNKNOWN); // Change INCORRECT_FLAGGED_MINE back to UNKNOWN
            num_incorrect_flagged_mine--;
        } else {
            setCell(row, col, INCORRECT_FLAGGED_MINE); // Flagged incorrectly
            num_incorrect_flagged_mine++;
        }
    } else if (action == 'r') {
        // Reveal a cell
        if (isMine(row, col)) {
            explode = true; // Set explode to true when a mine is revealed
            return;
        } else {
            reveal(row, col);
        }
    } else if (action == 'u') {
        // Undo flagging a cell
    ...
    // Check if the game is over after each command
    if (num_correct_flagged_mine == num_mine && num_incorrect_flagged_mine == 0) {
        explode = false; // No incorrectly flagged mines, all mines correctly flagged
    }
}
